...
class ColorHistogram {
    ...
    count_Pixels (pixels) {
        ...
        let histogram = []; //Array com o numero de pixels das 12 cores

        for(let i = 0; i < this.redColor.length; i++){
            histogram.push(0); //Insirir 0's
        }


        for(let i = 0; i < pixels.data.length; i += 4) {//Processar todos os pixeis
            let pR = pixels.data[i];    //R
            let pG = pixels.data[i + 1];//G
            let pB = pixels.data[i + 2];//B
            for (let j = 0; j < histogram.length; j++) {//Comparar com as 12 cores do histograma
                let r = Math.abs(this.redColor[j] - pR);
                let g = Math.abs(this.greenColor[j] - pG);
                let b = Math.abs(this.blueColor[j] - pB);
                if (r < this.ierror && g < this.ierror && b < this.ierror) {
                    if ((r + b + g) < this.gerror) {
                        histogram[j] += 1;//Se passar por todas as condicoes
                        break;            //Ja adicionou ao histograma, passa para o proximo pixel
                    }
                }
            }
        }
...
class ColorMoments {
    ...
    moments (imgobj, cnv) {
        ...
        let n = wBlock * hBlock;
        let descriptor = []; //este array guarda valores calculados
